...
angular.module('mw-datepicker-range').config(['$provide', '$injector', function ($provide, $injector) {
	...
	var datepickerDelegate = function datepickerDelegate($delegate, mwMultiSelectService) {
		...
		directive.compile = function () {
			return function (scope, elem, attrs, ctrls) {
				...
				var multi_select = void 0;
				var id = mwMultiSelectService.get_last_datepicker();
				if (attrs.mwMultiSelect === undefined) {
					var _multi_select_element = angular.element(document.getElementById(id));
					//watch for datepickerpopup
					multi_select = _multi_select_element.attr('mw-multi-select');
					if (multi_select !== undefined) {
						_multi_select_element.scope().$watchCollection(multi_select, function (value_new) {
				...
				if (multi_select !== undefined) {
					var change_listener = function change_listener() {
	...
	var datepickerPopupDelegate = function datepickerPopupDelegate($delegate, $filter, $timeout) {
		var directive = $delegate[0];
		...
		directive.compile = function () {
			return function (scope, elem, attrs, ctrls) {
				...
				if (multi_select !== undefined) {
					var set_view_value = function set_view_value(value) {
						multi_select = attrs.mwMultiSelect;
...
angular.module('mw-datepicker-range').directive('stDateRange', ['$filter', function ($filter) {
	return {
		...
		link: function link(scope, element, attr, table) {
...
angular.module("mw-datepicker-range-template", []).run(["$templateCache", function($templateCache) {
	$templateCache.put("stDateRange.html",
...
//# sourceMappingURL=mw-datepicker-range.js.map